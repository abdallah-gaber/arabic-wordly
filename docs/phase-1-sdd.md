...
- [ ] **Domain:** Create `ResultGridFormatter` service to parse game state (attempts vs correct word) to appropriate emojis.
- [ ] **Tests (Unit):** Ensure `ResultGridFormatter` returns valid emoji strings for various game states (perfect win, multi-attempt win, loss).
